...
list.linReg = lapply(mat.sym.ind , function(id){
  ...
  dd = data.frame(dd , ids=id, col=ci-1, row=ri-1)
  return(dd)
})
...
#                                      rightRelation=rtercen::prefix(relation=rtercen::relation(table.entryPoint$getName()),prefix=factorNS),
#                                      leftColumns=rtercen::addTablePrefix(table.join$getName(), 'row'),
#                                      rightColumns=rtercen::addTablePrefix(table.entryPoint$getName(), '_rids'))

composite.relation1 = rtercen::join1(leftRelation=composite.relation1,
                                     rightRelation=rtercen::relation(table.entryPoint2$getName()),
                                     leftColumns=rtercen::addTablePrefix(table.join$getName(), 'row'),
                                     rightColumns=rtercen::addTablePrefix(table.entryPoint2$getName(), '_rids'))

composite.relation1.json = composite.relation1$toTson()
...
composite.relation2 = rtercen::join1(leftRelation=rtercen::relation(table.entryPoint$getName()),
                                     ...
                                     leftColumns=rtercen::addTablePrefix(table.entryPoint$getName(), '_rids'),
